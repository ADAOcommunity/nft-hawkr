...
const SpaceBud = ({ pageContext: { spacebud } }) => {
  ...
  const CONTRACT_ADDRESS =
    "addr1w8dtruhr2eucu3fzedqnxr37230tl9qv5fzmyj5gckl3yug6muaq7";
  ...
  const loadSpaceBudData = async () => {
    // How can we make this look only for the information that is relevant to it.
    setIsLoadingMarket(true);
    setOwner([]); // This looks for the spacebud at the address that it is found at.
    ...
    let offerUtxo = await market.current.getOffer(spacebud.id); // This may or may not be a list. Which is fucked, ts ftw?
    // check if twin
